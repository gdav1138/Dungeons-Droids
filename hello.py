...
def getOutput(userInput, userId):
    ...
    while True:
        ...
        if cur_section == "Starting":
            ...
            return {"response": doSectionStarting(userId), "map": None, "minimap": None, "inventory": []}
        if cur_section == "GetPlayerName":
            print("Calling getplayerName")
            return {"response": doGetPlayerName(userInput, userId), "map": None, "minimap": None, "inventory": []}
        if cur_section == "GetPlayerStrength":
            print("Calling getPlayerStrength")
            return {"response": doGetPlayerStrength(userInput, userId), "map": None, "minimap": None, "inventory": []}
        if cur_section == "GetPlayerIntelligence":
            print("Calling getPlayerIntelligence")
            return {"response": doGetPlayerIntelligence(userInput, userId), "map": None, "minimap": None, "inventory": []}
        if cur_section == "ConfirmPlayerStats":
            print("Calling confirmPlayerStats")
            text_response = doConfirmPlayerStats(userInput, userId)
            # Check if we just entered MainGameLoop (character created)
            if all_global_vars.get_section(userId) == "MainGameLoop":
                map_html = all_global_vars.get_room_holder(userId).get_map_html(userId)
                minimap_html = all_global_vars.get_room_holder(userId).get_minimap_html(userId)
                return {"response": text_response, "map": map_html, "minimap": minimap_html, "inventory": get_inventory_data()}
            return {"response": text_response, "map": None, "minimap": None, "inventory": []}
        if cur_section == "MainGameLoop":
            text_response = do_main_loop(userInput, userId)
            map_html = all_global_vars.get_room_holder(userId).get_map_html(userId)
            minimap_html = all_global_vars.get_room_holder(userId).get_minimap_html(userId)
            return {"response": text_response, "map": map_html, "minimap": minimap_html, "inventory": get_inventory_data()}
